...
def score_player(player: PlayerState, database: CardDatabase) -> int:
    zones = player.deck + player.hand + player.discard + player.played + player.ongoing
    if player.familiar:
        zones.append(player.familiar)
    total = sum(database.cards[card_id].victory_points for card_id in zones if card_id in database.cards)
...
def compute_winners(state: GameState, database: CardDatabase) -> list[int]:
    ...
    if not ranked:
        return []
    best_key = (
        score_player(ranked[0], database),
